...
pub static GLOBAL_PARTIAL_CRC_COUNT: AtomicUsize = AtomicUsize::new(0);
pub static GLOBAL_FULL_CRC_COUNT: AtomicUsize = AtomicUsize::new(0);
pub static GLOBAL_FULL_READ_COUNT: AtomicUsize = AtomicUsize::new(0);
...
impl FdupesGroup {
    ...
    pub fn partialcrc(&mut self) -> io::Result<u16> {
        if let Some(crc) = self.partialcrc {
            ...
            GLOBAL_PARTIAL_CRC_COUNT.fetch_add(1, Ordering::SeqCst);
    ...
    pub fn fullcrc(&mut self) -> io::Result<u16> {
        if let Some(crc) = self.fullcrc {
            ...
            self.fullcrc = Some(crc);
            GLOBAL_FULL_CRC_COUNT.fetch_add(1, Ordering::SeqCst);
            Ok(crc)
        }
    }
}
...
impl PartialEq<FdupesGroup> for FdupesGroup {
    fn eq(&self, other: &FdupesGroup) -> bool {
        ...
        GLOBAL_FULL_READ_COUNT.fetch_add(2, Ordering::SeqCst);
